...
class HierarchicalMpc(object):
    ...
    def step(self):
        ...
        for ref_index, trajectory in enumerate(traj_list):
            ...
            rew, rew4rl = 0., 0.
            state_all = np.array((list(self.obs[:6 + 3 * (1 + self.num_future_data)]) + [0, 0]) * self.horizon + list(
                self.obs[:6 + 3 * (1 + self.num_future_data)])).reshape((-1, 1))
            ...
            traj_return_value.append(cost.squeeze().tolist())
            action_total.append(mpc_action)

        # choose
        path_index = np.argmin(traj_return_value)
        mpc_action = action_total[path_index]

        self.data2plot.append(dict(mpc_obs=self.obs,
                                   # rl_obs=obs4rl,
                                   mpc_action=mpc_action,
                                   # rl_action=rl_action,
                                   # rl_action_mpc=rl_action_mpc,
                                   mpc_time=self.cal_timer.mean * 1000,
        ...
        plt.plot([state[i][3] for i in range(1, self.horizon - 1)], [state[i][4] for i in range(1, self.horizon - 1)],
                 'r*')
        plt.show()
        plt.pause(0.001)
