...
class EnvironmentModel(object):  # all tensors
    ...
    def judge_dones(self, obses):
        ...
        for dones in [dones_alpha_f, dones_alpha_r, dones_r]:
        ...
        if self.task == 'left':
            ...
            for ego_point in [ego_front_points, ego_rear_points]:
                ...
                for dones in [dones_before1, dones_before2, dones_middle1, dones_middle2, dones_middle3, dones_middle4,
                              ...
                              dones_after1, dones_after2, dones_good_done]:
                    self.dones = tf.math.logical_or(self.dones, dones)
    ...
    def compute_rewards2(self, obses, actions, prev_dones):
        with tf.name_scope('compute_reward') as scope:
            ...
            rewards = 0.01 * devi_v + 0.04 * devi_y + 5 * devi_phi + 0.02 * punish_yaw_rate + \
                      0.05 * punish_steer + 0.0005 * punish_a_x + 1000 * veh2road + veh2veh + \
                      100 * rew_alpha_f + 100 * rew_alpha_r + 100 * rew_r
            rewards = tf.cast(tf.math.logical_not(prev_dones), tf.float32) * rewards
            return rewards
    ...
    def compute_next_obses(self, obses, actions):
        ego_infos, tracking_infos, veh_infos = obses[:, :12], obses[:, 12:12 + 3 + 3 * self.num_future_data], \
                                               obses[:, 12 + 3 + 3 * self.num_future_data:]
        ego_next_infos_except_lw, ego_next_params = self.vehicle_dynamics.prediction(ego_infos[:, :6], actions, self.base_frequency, 1)
        ...
        next_obses = tf.concat([ego_next_infos_except_lw, ego_next_lw, ego_next_params, next_tracking_infos, next_veh_infos], 1)
        return next_obses
    ...
    def render(self, mode='human'):
        if mode == 'human':
            ...
            ego_info, tracing_info, vehs_info = obses[0, :12], obses[0, 12:12 + 3 + 3 * self.num_future_data], \
                                                obses[0, 12 + 3 + 3 * self.num_future_data:]
            # plot cars
            ...
            ego_l = ego_info[6]
            ego_w = ego_info[7]
            ego_alpha_f = ego_info[8]
            ego_alpha_r = ego_info[9]
            ego_miu_f = ego_info[10]
            ego_miu_r = ego_info[11]
            ...
            plt.text(text_x, text_y_start - 3 * next(ge),
            ...
            # done info
            plt.text(text_x, text_y_start - 3 * next(ge), 'done info: {}'.format(self.dones[0]))
